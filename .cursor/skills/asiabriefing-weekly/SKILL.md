...
description: Retrieves recent Asia Briefing posts (all regions/sub-sites, any topic) from the last 5 days, summarizes them into a list with links and key points, and adds sales-oriented insights. Output goes to output/weekly_new/. Use when the user asks for "Asia Briefing 本周新文", "weekly new posts", "最近几天 Asia Briefing 汇总", or "weekly_new 更新".
---

# Asia Briefing 本周新文汇总

从 Asia Briefing 及子站检索**最近 5 天（或本周）**的新文章，汇总成清单写入 `output/weekly_new/`，并延伸销售向洞察。
...
- **目录**：`output/weekly_new/`
- **文件名**：`YYYY-MM-DD-weekly-new-posts.md`（以执行日期命名）。
- **禁止**：不得从 `output/` 下读取任何文件作为输入；本技能只写不读。
...
- **output 规则**：仅向 `output/weekly_new/` 写入，不从 output 读取任何内容。
